...
Jgl_SpriteList.prototype.collision = function(sprite1, sprite2, fuzziness, circular) {
    if (sprite1 && sprite2) {
        ...
        if (circular) {
            var xd = sprite2.collisionRect.x - sprite1.collisionRect.x;
            var yd = sprite2.collisionRect.y - sprite1.collisionRect.y;
            ...
            return ( Math.sqrt(xd * xd + yd * yd) < rad );
        } else {
...
//***********************************************
Jgl_Sprite.prototype.draw = function(ctx, xp, yp) {
gctx = ctx;
    if (ctx){
        if (xp) {
            this.x = xp;
        ...
        if (this.center) {
            // Use this post-calculated center/radius for doing collision detection
            this.collisionRect = { x: x, y: y, w: width, h: height };
        ...
        if (this.active) {
            ...
            if (this.rotationRadians) {
                ctx.save();
                ctx.translate(finalX + parseInt(width / 2), finalY + parseInt(height / 2));
                finalX = -parseInt(width / 2);
                finalY = -parseInt(height / 2);
...
//***********************************************
Jgl_Sprite.prototype.setScale = function(scale) {
    this.scale = scale;
};
...
//***********************************************
Jgl_Sprite.prototype.setPosition = function(x, y, rotation) {
    if (typeof(x) == 'number') {
        this.x = x;
    }
    if (typeof(y) == 'number') {
